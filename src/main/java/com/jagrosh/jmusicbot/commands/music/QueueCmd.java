...
public class QueueCmd extends MusicCommand {
    ...
    public void doCommand(CommandEvent event) {
        ...
        if (list.isEmpty()) {
            ...
            MessageCreateData built = new MessageCreateBuilder()
                    .setContent(event.getClient().getWarning() + " 隊列中沒有音樂!")
    ...
    private String getQueueTitle(AudioHandler ah, String success, int songslength, long total, RepeatMode repeatmode, QueueType queueType) {
        ...
        return FormatUtil.filter(sb.append(success).append(" 目前隊列 | ").append(songslength)
                .append(" 首 | `").append(TimeUtil.formatTime(total)).append("` ")
